...
class SUBNet(bp.Container):
  def __init__(self, spike,scale=1.0,num=1,method='exp_auto',device = devices[1]):
    super().__init__()
  ...
  def update(self,t,dt,spike,*args, **kwargs):
    ...
    input = 20.
    self.neurons.input+=self.E.update(t,dt)+self.I.update(t,dt)+input
    self.neurons.update(t,dt)
    return self.neurons.spike.value

# neu1 = InputNeurons(spike)
# neu2 = LIF(2,device=devices[0])
# synapse = Exponential(pre=neu1,post=neu2)
spike1 = jax.device_put(bm.ones(1,dtype=bool),devices[0])
spike2 = jax.device_put(bm.ones(1,dtype=bool),devices[0])
net1 = SUBNet(spike=spike2,device=devices[1])
net2 = SUBNet(spike=spike1,device=devices[0])
during = 1
t = 0
dt = 0.4
...
while t < during:
    ...
    # neu1.input+=20.
    # neu2.input+=20.
    # neu2.input+=synapse.update(t,dt)
    # neu1.update(t,dt)
    # neu2.update(t,dt)
    # neu2.update(t,dt, input=synapse.update)
    t = t + dt
    # print(neu2.V.value)
    print(spike1.device_buffer.device())
    print(spike2.device_buffer.device())
    # print(b.device_buffer.device())
    # print(a)
    # print(b)